...
import com.dvc.mybilibili.danmaku.live.parser.SocketLotteryParserProcess;
...
public class CallbackDispatchRunnable implements Runnable {
    private static final String TAG = CallbackDispatchRunnable.class.getSimpleName();
    ...
    private void initMsgParser() {
        ...
        addMsgParser(new SocketLotteryParserProcess());
    }
    ...
    private SocketMsgParserProcess getMsgParser(String cmd) {
        ...
        if(parserMaps.containsKey(ccmd)) {
            return parserMaps.get(ccmd);
        }
        return null;
    }
    ...
    private void dispatch() throws Exception {
        ...
        if (Arrays.equals(protocolBytes, PackageRepository.DAN_MU_DATA_PACKAGE_PROTOCOL_BYTES)) {    //json数据包
            ...
            RxLogTool.d(TAG+":danmu", new String(jsonBytes));
            ...
            RxLogTool.d(TAG+":Viewers: " + onlineCount);
            ...
            RxLogTool.d(TAG+":Unknown package below", new String(jsonBytes));
        }
        if (processer != null) {
    ...
    public void run() {
        while (true) {
            try {
                ...
                if (liveDanMuReceiver.isConnected()) {
                    break;
                }
            } catch (JSONException e) {
                RxLogTool.e(TAG+":danmu", "Wrong JSON: " + new String(jsonBytes));
                e.printStackTrace();
            } catch (Exception e) { //其他错误时显示错误信息并继续监听下一个数据包
                RxLogTool.e(TAG+":danmu", "another wrong");
                e.printStackTrace();
            }
        }
