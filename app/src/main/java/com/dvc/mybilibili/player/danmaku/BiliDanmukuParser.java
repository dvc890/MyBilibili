        ...
        public void startElement(String uri, String localName, String qName, Attributes attributes)
            ...
            if (tagName.equals("d")) {
                ...
                if (values.length > 0) {
                    ...
                    long time = Long.valueOf(values[2]); // 出现时间
                    int type = Integer.parseInt(values[3]); // 弹幕类型
                    float textSize = Float.parseFloat(values[4]); // 字体大小
                    int color = (int) ((0x00000000ff000000 | Long.parseLong(values[5])) & 0x00000000ffffffff); // 颜色
                     int poolType = Integer.parseInt(values[7]); // 弹幕池类型（忽略
                    item = mContext.mDanmakuFactory.createDanmaku(type, mContext);
                    if (item != null) {
                        item.userHash = values[8];
                        item.setTime(time);
